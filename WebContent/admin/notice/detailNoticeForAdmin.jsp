...
<div class="wrap"> 
 ...
 <h2>${ndto.title }</h2>
	...
	<c:if test="${isEvent == 1 }"><!-- 투표가 있는 이벤트인지 -->
	  <div class="barGraph">
		  투표 현황
		    <ul class="graph">
		      <span class="graph-barBack">		      	
		      <c:forEach var="nlistDTO" items="${nlist }">
			        <li class="graph-bar" data-value="${nlistDTO.score }">
			          <span class="graph-legend">${nlistDTO.nomiName }</span>
			        </li>
		      </span>  
			</c:forEach>
...
<a class="btn btn-primary" onclick="javascript:goOtherEvents()">공지사항 목록</a>
${isEvent }
<c:forEach var="noticeData" items="${nlist }">
	<c:if test="${isEvent==0 }">
	<form action="detail.notice?ntnum=${noticeData.ntnum }&isEvent=0&eventnum=${noticeData.eventnum}&isUpdate=0" method="post" name="fupdate">
	   <a class="btn btn-primary"
	      onclick="javascript: document.fupdate.submit()">수정0</a>
	</form>
	</c:if>
	
	<c:if test="${isEvent==1 }">
	<form action="detail.notice?ntnum=${noticeData.ntnum }&isEvent=1&eventnum=${noticeData.eventnum}&isUpdate=1" method="post" name="fupdate">
	   <a class="btn btn-primary"
	      onclick="javascript: document.fupdate.submit()">수정1</a>
	</form>
	</c:if>
</c:forEach>


<form action="delete.notice?ntnum=${ndto.ntnum}" method="post" name="fdelete">
   <a class="btn btn-primary"
      onclick="javascript: document.fdelete.submit()">삭제</a>
</form>
